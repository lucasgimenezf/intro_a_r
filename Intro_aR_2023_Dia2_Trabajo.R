...
library(tidyverse) #debe hacer correr cada vez que se va usar, llamar al paquete
...
##### ANOVA ##### 
# Subir datos
DPS <- read.csv("DesempenoProductivo_Soja.csv",dec=",")

#dec igual entre comillas para actualizar la tabla a coma en vez de punto
getwd() #para encontrar la ubicacion de la carpeta

#Los nombres de columnas tienen que seguir las buenas practicas 

colnames(DPS) #Explorar tabla
head(DPS) #Nomvbre de columnas
unique(DPS)(Variedad)#primeros datos de la tabla

DPS$Peso1000semillas_g * DPS$NGranos_m2
...
# Realizar el ANOVA
modelo_anova <- aov(NGranos_m2 ~ Variedad, data = DPS)

# Resumen del ANOVA
summary(modelo_anova)

# Parcelas agrupadas por variedad
boxplot(NGranos_m2 ~ Variedad, data =DPS,
    #Base R
        
         
 xlab = "Variedad", #nombre eje x
 ylab = "Numero de granos",
 main = "Rendimientos de cultivos por variedad",
 las = 2, #para rotar,
 cex.names = 1 )
...
aggregated_data <- aggregate(Rendimiento_Kgha ~ NGranos_m2, data = DPS, FUN = mean )


#Agregar linea
ggplot(DPS, aes(x = NGranos_m2, y = Rendimiento_Kgha )) +
  geom_point() +
  geom_smooth(data = aggregated_data, 
              aes(x = NGranos_m2, y = Rendimiento_Kgha),
              method = "lm", se = FALSE) +
  ggtitle("Rendimiento Kg/ha vs. Numero de Granos por m2") +
  xlim(0, 4000)
...
# Crear modelo linear
lm_fit <- lm( Rendimiento_Kgha ~ NGranos_m2, data = aggregated_data )
#lm es para modelos lineales

# Sacar coeficientes
intercept <- coef(lm_fit)[1]    #intercepto
slope <- coef(lm_fit)[2] #slope = pendiente

# armar equacion
#Que hace la funcion paste?
equation <- paste("y =", #crear caracteres
                  round(slope, 2), #redondeo
                  "x +", round(intercept, 2))

# Plot the scatter plot with a regression line fitted to aggregated data
a <- ggplot(DPS, aes(x = NGranos_m2 , y = Rendimiento_Kgha )) +
  geom_point() +
  geom_smooth(data = aggregated_data,
              aes(x = NGranos_m2 , y = Rendimiento_Kgha ),
              method = "lm", se = FALSE) + 
